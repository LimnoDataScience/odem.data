...
compare_predict_versus_observed<-function(obs,input.values){
  ...
  for(jj in 1:length(ndate)){
    ...
    if(td_not_exist[kk]){
      ...
      td <- input.values$thermocline_depth[kk]
      test_data$predict_epil_do[jj] <- input.values$o2_epil[kk]/input.values$volume_epi[kk]/1000
      test_data$predict_hypo_do[jj] <- input.values$o2_hypo[kk]/input.values$volume_hypo[kk]/1000
  ...
  fit = calc_fit(input.values , proc.obs = val_data )
  return(test_data)
}
